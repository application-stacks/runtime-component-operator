...
You can easily perform day-2 operations on an instance of `RuntimeComponent` using the `RuntimeOperation` custom resource (CR), which allows you to specify the commands to execute on a container.
...
| `containerName` | The name of the container within the Pod. Defaults to main container `app`.
| `command`       | Command to execute. It's not executed within a shell.
|===
...
You can check the status of a runtime operation using the `status` field inside the CR YAML. You can also run the command `oc get runtimeop -o wide` to see the status of all operations in the current namespace.

NOTE: The `RuntimeOperation` CR must be created in the same namespace as the `RuntimeComponent` to operate on. Once the `RuntimeOperation` has started, the CR can not be re-used for more operations. A new CR needs to be created for each day-2 operation. The operator can process only one `RuntimeOperation` at a given time. Long running commands can queue up the other runtime operations.
