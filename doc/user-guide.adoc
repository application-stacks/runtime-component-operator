...
| `bindings.expose.enabled` | A boolean to toggle whether the operator should expose the application as a bindable service. The default value for this parameter is `false`.
...
According to the specification, a Provisioned Service resource must define a `.status.binding.name` which is a `LocalObjectReference`-able to a Secret.
...
Once a `RuntimeComponent` application is exposed as a Provisioned Service, a `ServiceBindingRequest` can refer to the application as a backing service.
