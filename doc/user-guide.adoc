...
To deploy a Docker image containing a runtime component to a Kubernetes environment you can use the following CR:
...
In order for the Runtime Component Operator to pass the binding information to the application, you can specify the name of the `ServiceBindingRequest` as follows:
...
Once the Service Binding Operator processes the `my-binding` custom resource of the type `ServiceBindingRequest`, it creates a `Secret` containing binding information provided by the backing service. The Runtime Component Operator would inject the `Secret` as environment variables into application pods.

The Runtime Component Operator can also automatically detect if there is a `ServiceBindingRequest` custom resource with the same name as the application in the application namespace. If the operator detects the custom resource, it injects the binding secret into the application container.

Users can turn off the auto-detection mechanism by setting the `bindings.autoDetect` parameter to `false`. The `bindings.resourceRef` parameter takes precedence over the `bindings.autoDetect` parameter. In other words, the Runtime Component Operator, injects secrets from the `my-binding` resource, even if there is a `ServiceBindingRequest` resource named `my-app`:
