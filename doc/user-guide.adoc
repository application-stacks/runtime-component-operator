...
The YAML definition specified in the `bindings.embedded` parameter must not include a `metadata`section as the Runtime Component operator adds a `metadata`section. If the YAML definition does not include `apiVersion` or `kind`, the Runtime Component operator uses the default `GroupVersionKind` value specified in the Operator Config Map.
