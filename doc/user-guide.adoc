...
Before you can use the Ingress resource to expose your cluster, you must install an ingress controller, such a Nginx or Traeik..

_The Ingress resource is created only if the `Route` resource is not available._


To use the Ingress resource, set the `defaultHostName` variable in the runtime-component-operator ConfigMap object to a host name such as mycomp.com
