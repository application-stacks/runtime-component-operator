...
func getMaxReconcileInterval(success bool) int {
	var maxInterval int
	if success {
		maxInterval, _ = strconv.Atoi(common.LoadFromConfig(common.Config, common.OpConfigRreconcileIntervalSuccessMaximum))
	} else {
		maxInterval, _ = strconv.Atoi(common.LoadFromConfig(common.Config, common.OpConfigRreconcileIntervalFailureMaximum))
	}
...
func resetReconcileInterval(s common.BaseComponentStatus, ba common.BaseComponent) time.Duration {
	...
	if ba.GetReconcileIntervalHide() != nil && *ba.GetReconcileIntervalHide() {
