...
NOTE: Any references to *Api Version* `apps.openliberty.io/v1beta2` must be replaced with `rc.app.stacks/v1`, *Kind* `OpenLibertyApplication` with `RuntimeComponent`, and *ClusterRole* `openlibertyapplications.apps.openliberty.io-v1beta2` with `runtimecomponents.rc.app.stacks-v1beta2`.
