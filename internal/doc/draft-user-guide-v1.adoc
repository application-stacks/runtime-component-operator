...
| `networkPolicy.disable` | A boolean to toggle the creation of the network policies resources. Defaults to false.
...
| `storage.className`| A convenient field to set the storage class of the persisted storage. Can be overridden by the `storage.volumeClaimTemplate` property.
...
| `securityContext` | Security context to control privilege and permission settings for application container. See link:++https://kubernetes.io/docs/tasks/configure-pod-container/security-context/#set-the-security-context-for-a-container++[Security Context for a Container].
...
A security context is used to control privilege and permission settings for a Pod or a Container. By default, the Runtime Component operator sets several security context parameters for application container as shown below.
...
The security context parameters are customizable under `.spec.securityContext` field in the `RuntimeComponent` CR. For example, you can override the default values or set additional parameters:
...
For more information on security context parameters, see link:++https://kubernetes.io/docs/reference/generated/kubernetes-api/v1.19/#securitycontext-v1-core++[SecurityContext].

NOTE: If your Kubernetes cluster does not generate a user ID and `securityContext.runAsUser` is not specified, the user ID defaults to the value in the image metadata. If the image does not have a user ID specified either, you will have to assign a user ID through  `securityContext.runAsUser` to meet `securityContext.runAsNonRoot` requirement.
...
With the `RuntimeComponent` CR definition below the operator will create `PersistentVolumeClaim` called `pvc` with the size of `1Gi`, `ReadWriteOnce` access mode and use the cluster's default storage class.
...
The persistent storage and PersistentVolumeClaim cannot be added or changed once StatefulSet is created.
...
Network policies for an application are created by default to isolate incoming traffic. To disable creating network
policies for an application, set `.spec.networkPolicy.disable` to `true`:
...
OpenShift provides support for network policies by default. Users deploying to other Kubernetes platforms will need to
ensure that their network plugin supports network policies as described
link:https://kubernetes.io/docs/concepts/services-networking/network-policies/#prerequisites[here].

==== Default Network Polices

* The default network policy created for applications that are not exposed limits incoming traffic to pods in the same
namespace that are part of the same application. Traffic is limited to only the ports configured by the service.
* For exposed applications on OpenShift, the network policy allows incoming traffic from the OpenShift ingress
controller on the ports configured in the service.
* For exposed applications on other Kubernetes platforms, incoming traffic is allowed from any pods in any namespace on the
ports configured in the service.
* On OpenShift, incoming traffic is also allowed from the OpenShift monitoring stack.

==== Configuring the Network Policy

The network policy can be configured to allow either certain namespaces and/or pods using
`.spec.networkPolicy.namespaceLabels` and `.spec.networkPolicy.fromLabels`, respectively.

By default, `.spec.networkPolicy.namespaceLabels` is set to the same namespace that the application is deployed to, and
`.spec.networkPolicy.fromLabels` is set to select pods that belong to the same application specified by
`.spec.applicationName`.

Example of allowing incoming traffic from pods labeled with `role: frontend` and are in the same namespace:
...
Example of allowing incoming traffic from pods belonging to the same application in the `example` namespace:
...
Example of allowing incoming traffic from pods labeled with `role: frontend` in the `example` namespace:
