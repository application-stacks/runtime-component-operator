...
| `networkPolicy.namespaceLabels` | A set of labels that selects the namespace to allow ingress traffic from. Defaults to the same namespace that the application is deployed to.
| `networkPolicy.fromLabels` | A set of labels that selects the pod(s) to allow ingress traffic from. Defaults to pod(s) belonging to the same application.
...
Network policies for an application are created by default to isolate ingress traffic. To disable creating network
...
* The default network policy created for applications that are not exposed limits ingress traffic to pods in the same
namespace that are part of the same application. Traffic is limited to only the ports configured by the service.
* For exposed applications on OpenShift, the network policy limits ingress traffic from the OpenShift ingress
controller on the ports configured in the service.
* For exposed applications on other Kubernetes platforms, ingress traffic is allowed from any pods in any namespace on the
ports configured in the service.
* On OpenShift, ingress traffic is also allowed from the OpenShift monitoring stack.
...
Example of allowing ingress traffic from pods in the same namespace labeled with `role: frontend`:
...
Example of allowing ingress traffic from pods belonging to the same application in the `example` namespace:
...
Example of allowing ingress traffic from pods labeled with `role: frontend` in the `example` namespace:
