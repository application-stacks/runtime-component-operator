...
| `securityContext` | Security context to control privilege and permission settings for a container. See link:++https://kubernetes.io/docs/tasks/configure-pod-container/security-context/#set-the-security-context-for-a-container++[Security Context for a Container].
...
A security context is used to control privilege and permission settings for a Pod or a Container. By default, the Runtime Component operator sets several security context parameters for containers as shown below.
