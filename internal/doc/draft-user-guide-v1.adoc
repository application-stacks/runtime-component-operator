...
| `networkPolicy.namespaceLabels` | A set of labels that selects the namespace to allow ingress traffic from.
| `networkPolicy.fromLabels` | A set of labels that selects the pod(s) to allow ingress traffic from.
...
Example of allowing ingress traffic from pods in any namespace labeled with `role: frontend`:
...
Example of allowing ingress traffic from any pod in the `example` namespace:
