...
To install using the integrated OperatorHub in OpenShift UI, select `Operators` from the navigation panel, then select `OperatorHub` and then search for `Runtime Component`. Click on the tile with `Runtime Component` to install the Operator. Select the channel `v1.4` and install.
  ...
  * (Optional) To specify more than one worker that can reconcile Runtime Component CR instances concurrently, select `Manual` Update Approval and install the operator. Under the `Installed Operators` tab, you should see the operator pending installation with message "Upgrade available". Click the name to enter the Subscription resource then navigate to the YAML tab to update the Subscription's `.spec` field with the following configuration:
...
```
    ...
    * (Optional) To specify more than one worker that can reconcile Runtime Component CR instances concurrently, set the environment variable `MAX_CONCURRENT_RECONCILES`
