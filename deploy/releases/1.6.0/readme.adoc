= Runtime Component Operator v1.6.0
...
Red Hat OpenShift 4.16 or higher is supported. The operator is also supported on Kubernetes environments with v1.29 or higher.
...
To install using the integrated OperatorHub in OpenShift UI, select `Operators` from the navigation panel, then select `OperatorHub` and then search for `Runtime Component`. Click on the tile with `Runtime Component` to install the Operator. Select the channel `v1.6` and install.
